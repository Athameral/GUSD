...
from .gusd_sampling import sampling_4d
from ..checkpoint import checkpoint as cp
from ..csrc.wrapper import MSMV_CUDA


@TRANSFORMER.register_module()
class GUSDTransformer(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
        ...
        self.decoder = GUSDTransformerDecoder(
            embed_dims, num_frames, num_views, num_points, num_layers, num_levels,
            num_refines, num_groups, num_pt_channels, num_classes, scales, pc_range=pc_range,
            output_all_gs=output_all_gs, use_gs_param_encoder=use_gs_param_encoder,
            debug=debug)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        # cls_scores[i] is either None or (gs_params, semantics) tuple
        cls_scores = [
            None if s is None else (
                torch.nan_to_num(s[0]),  # gs_params
                torch.nan_to_num(s[1])   # semantics
            )
            for s in cls_scores
        ]
...
class GUSDTransformerDecoder(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
        ...
        # self.pt_feat_branch = nn.Linear(embed_dims, num_pt_channels * num_refines[-1])
        for i in range(num_layers):
            self.decoder_layers.append(
                GUSDTransformerDecoderLayer(
                    embed_dims, num_frames, num_views, num_points, num_levels,
                    num_groups, num_pt_channels, num_refines[i], before_refines[i],
                    num_classes=num_classes,
                    last_layer=i==num_layers-1, scale=scales[i], pc_range=pc_range,
                    output_all_gs=output_all_gs, use_gs_param_encoder=use_gs_param_encoder)
            )
...
class GUSDTransformerDecoderLayer(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_views=6,
        ...
        if (not self.last_layer) and use_gs_param_encoder:
            self.gs_param_encoder = nn.Sequential(
                nn.Linear((self.ga_geom_dim + self.num_classes) * self.num_refines, self.embed_dims),
                nn.LayerNorm(self.embed_dims),
                nn.ReLU(inplace=True),
                nn.Linear(self.embed_dims, self.embed_dims),
                nn.LayerNorm(self.embed_dims),
            ...
            self.gs_param_encoder = None


        self.self_attn = GUSDSelfAttention(
            embed_dims, num_heads=8, dropout=0.1, pc_range=pc_range)
        self.sampling = GUSDSampling(embed_dims, num_frames=num_frames, num_views=num_views,
        ...
        # Output: (ga_geom_dim + num_classes) per refinement, for Gaussian Splatting
        cls_branch.append(nn.Linear(self.embed_dims, (self.ga_geom_dim + num_classes) * self.num_refines))
    ...
    def init_weights(self):
        ...
        if self.gs_param_encoder is not None:
            for m in self.gs_param_encoder.modules():
    ...
    def forward(self, query_points, query_feat, mlvl_feats, occ2img, img_metas):
        ...
        cls_score = None
        if self.training or self.last_layer or self.output_all_gs:
            raw_output = self.cls_branch(query_feat)  # (B, Q, (8+C)*R)
            raw_output = raw_output.reshape(B, Q, self.num_refines, self.ga_geom_dim + self.num_classes)
            gs_params = raw_output[..., :self.ga_geom_dim]   # (B, Q, R, 8)
            semantics = raw_output[..., self.ga_geom_dim:]   # (B, Q, R, C)
            cls_score = (gs_params, semantics)  # 打包为元组返回

            # 在当前层结尾编码输出 GS 属性, 注入 query_feat 传递给下一层
            # GSPE
            if (not self.last_layer) and self.gs_param_encoder is not None:
                gs_flatten = torch.cat([gs_params, semantics], dim=-1).flatten(2, 3)  # (B, Q, R*(8+C))
                gs_feat = self.gs_param_encoder(gs_flatten.detach())
                query_feat = query_feat + gs_feat

        return query_feat, cls_score, refine_pt


class GUSDSelfAttention(BaseModule):
