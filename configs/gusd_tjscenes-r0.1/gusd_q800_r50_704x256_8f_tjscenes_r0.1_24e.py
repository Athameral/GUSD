dataset_type = 'TJScenesOcc3DDataset'
dataset_root = 'data-tjscenes/v0.0.4_r0.1/'
occ_root = 'data-tjscenes/v0.0.4_r0.1/gts'
...
point_cloud_range = [-20.0, -20.0, -2.0, 20.0, 20.0, 4.4]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        scale_max_safe=0.7,
        gs_seed_scale=False,
        scale_range=[0.01, 0.7],
        cuda_kwargs=dict(
            scale_multiplier=3,
            H=400, W=400, D=64,
            pc_min=point_cloud_range[:3],
            grid_size=voxel_size[0],
            # chunk_num=(8, 8, 8),  # H×W×D 切成 2×2×1 块
            # chunk_margin=10,
        ),
        transformer=dict(
            type='GUSDTransformer',
            # Gaussian Parameter Encoder
    ...
    train_cfg=dict(
        pts=dict(max_gs_loss_samples=400000),
    ),
    test_cfg=dict(
        pts=dict(score_topk=360000),
    )
)
...
train_pipeline = [
    ...
    dict(type='LoadMultiViewImageFromMultiSweepsTJScenes', sweeps_num=num_frames - 1),
    # dict(type='LoadAnnotations3D', with_bbox_3d=True, with_label_3d=True, with_attr_label=False),
    dict(type='LoadOcc3DFromFileTJScenes', occ_root=occ_root), 
...
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', to_float32=False, color_type='color'),
    dict(type='LoadMultiViewImageFromMultiSweepsTJScenes', sweeps_num=num_frames - 1, test_mode=True),
    dict(type='RandomTransformImage', ida_aug_conf=ida_aug_conf, training=False),
    dict(
        type='MultiScaleFlipAug3D',
        img_scale=(ida_aug_conf['H'], ida_aug_conf['W']),
        pts_scale_ratio=1,
        flip=False,
        transforms=[
...
data = dict(
    ...
    train=dict(
        ...
        ann_file=dataset_root + 'tj-0.0.5_r0.1_multi_T01-T03-T05-T06-T07-T08-T09_occ_train.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=dataset_root,
        ann_file=dataset_root + 'tj-0.0.5_r0.1_multi_T01-T03-T05-T06-T07-T08-T09_occ_val.pkl',
...
total_epochs = 24
batch_size = 8 # bs=1 per gpu

# load pretrained weights
load_from = 'pretrain_v3/opusv2_s_100e.pth'
revise_keys = None
