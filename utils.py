...
def backup_code(work_dir, config_file=None, verbose=False):
    base_dir = os.path.dirname(os.path.abspath(__file__))
    for pattern in ['*.py', 'models/**/*.py', 'loaders/**/*.py']:
        for src in glob.glob(os.path.join(base_dir, pattern), recursive=True):
            file = os.path.relpath(src, base_dir)
    ...
    # backup only the current config file, not the entire configs/ directory
...
def _backup_wrapper(work_dir, config_file=None, verbose=False):
    try:
        backup_code(work_dir, config_file=config_file, verbose=verbose)
        logging.info('Async code backup finished: %s' % work_dir)
    except Exception:
        logging.exception('Async code backup failed')


def backup_code_async(work_dir, config_file=None, verbose=False):
    """Start a background daemon thread to backup code into `work_dir/backup`.

    Returns the Thread object so the caller can join if desired. Logs exceptions
    from the background thread.
    """
    try:
        thread = threading.Thread(target=_backup_wrapper, args=(work_dir, config_file, verbose), daemon=True)
        thread.start()
        if verbose:
            logging.info('Started background backup thread: %s' % thread.name)
        return thread
    except Exception:
        logging.exception('Failed to start backup thread')
        return None
...
class MyTextLoggerHook(TextLoggerHook):
    def _log_info(self, log_dict, runner):
        ...
        # Debug-prefixed keys: always print regardless of mode
        debug_keys = {k: v for k, v in log_dict.items() if k.startswith('debug')}
